...
class ZestorHelper:
    ...
    def scrape_google(query):
        ...
        try:
            ...
            print('scrape_google() Exception: %s' % (oops))
        return links
    ...
    def relevant_summary(SELECTED_AI_ENGINE, text):
        retval = ''
        try:
            print('relevant_summary:tokenizer')
            # Natural Language Toolkit line parser https://www.nltk.org/
            # https://www.nltk.org/_modules/nltk/tokenize/punkt.html
            tokenizer = nltk.data.load('tokenizers/punkt/english.pickle')
            print('relevant_summary:textLines')
            ...
            tempText = ''
            if len(textLines) > 0:
                print('relevant_summary:textLines>0')
                for line in textLines:
                    tempText += line # add a sentence to text
                    # when text > 2k chars, summarize
                    if len(tempText) > 2000:
                        print('relevant_summary:prompt')
                        prompt = ZestorHelper.open_file('PromptTemplates/' + SELECTED_AI_ENGINE + '/relevant_summary.txt').replace('<<TEXT>>',tempText)
                        print('relevant_summary:openai_callout')
                        retvalTemp = ZestorHelper.openai_callout(prompt, engine='text-davinci-002', temp=0)
                        print(type(retvalTemp))
                        print('relevant_summary:openai_callout')
                        retval += retvalTemp # add summary to retval
                        tempText = '' # clear out the temp variable
                # summarize text, when partial text block <= 2k chars at the end
                if len(tempText) > 0 :
                    print('relevant_summary:prompt#2')
                    prompt = ZestorHelper.open_file('PromptTemplates/' + SELECTED_AI_ENGINE + '/relevant_summary.txt').replace('<<TEXT>>',tempText)
                    print('relevant_summary:openai_callout#2')
                    retvalTemp = ZestorHelper.openai_callout(prompt, engine='text-davinci-002', temp=0)
                    retval += retvalTemp # add summary to retval
            else: 
                retval=text
        except Exception as oops:
            print('relevant_summary() Exception: %s' % (oops))
        # combined summary
        return retval 
    ...
    def get_url_only_text(url):
        ...
        try:
            ...
            rawFile = ZestorHelper.urldb_getfilename(url) + '.raw.txt'
            ZestorHelper.save_file(rawFile, responseHtml)

            #print('\n\n==Response==> %s' % (responseHtml)) 
            clean_response = ZestorHelper.clean_html(responseHtml)

            cleanFile = ZestorHelper.urldb_getfilename(url) + '.clean.txt'
            ZestorHelper.save_file(cleanFile, clean_response)

            print('\n\n==Clean Response==> %s' % (clean_response)) 
        except Exception as oops:
            print('get_url_only_text() Exception: %s' % (oops))
    ...
    def urldb_getfilename(url):
        ...
        try:
            ...
            print('urldb_getfilename() Exception: %s' % (oops))
        return './url_db/' + url_db_file
